...
class Page(object):
    ...
    legal = info_hider(
        "Legal note: CZ Manufacturing is a trading name of ANNELLA s.r.o. - a company registered in Czechia; sidlem Voronezska 144/20, 460 01 Liberec, ICO: 28706951, spisova znacka: C 27756 vedena u Krajskeho soudu v Usti nad Labem."
    )
    ...
    def generate_site(self):
        with open(PATH_TO_TEMPLATES.joinpath("page.html")) as tem_han:
            ...
            html_str = template.render(**dict(self), link_to_homepage=link_to_homepage)
            return html_str
...
pages = [
    Page(
        title="CZ Manufacturing: Industrial solutions",
    ...
    Page(
        title="CZ Manufacturing: About us",
    ...
    Page(
        title="CZ Manufacturing: Services",
        name="Our Services",
    ...
    Page(
        title="CZ Manufacturing: Contact",
    ...
    Page(
        title="CZ Manufacturing: How we meet standards",
    ...
    Page(
        title="CZ Manufacturing: Industrial Automation and Smart Manufacturing Services",
...
with open(PATH_TO_TEMPLATES.joinpath("site_map.xml")) as tem_han:
    template = Environment(loader=FileSystemLoader(PATH_TO_TEMPLATES)).from_string(
        tem_han.read()
    )
